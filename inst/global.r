...
hPlot_2axis<-function (..., axis_value, radius = 3, title = NULL, subtitle = NULL, group.na = NULL) 
{
  ...
  if (!is.null(d$group)) {
    ...
    plyr::ddply(data, .(group), function(x) {
      ...
      i <- which(groups == g)
      x$group <- NULL
      rChart$series(data = toJSONArray2(x, json = F, names = F), 
                    name = g, type = types[[i]], marker = list(radius = radius))
      #rChart$series(data = toJSONArray2(x, json = F, names = F), 
      #              name = g, type = types[[i]], yAxis=axis_value[[i]],marker = list(radius = radius))
      return(NULL)
    })
  }
  else {
  ...
  if (is.categorical(data$x)) {
    rChart$xAxis(title = list(text = d$x), categories = unique(as.character(data$x)), 
                 replace = T)
  }
  else {
  ...
  if (is.categorical(data$y)) {
    #rChart$yAxis(title = list(text = d$y), categories = unique(as.character(data$y)), 
    #             replace = T)
  }
  else {
